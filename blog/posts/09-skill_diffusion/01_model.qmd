...
```{r data-preparation}
data_path <- "C:/Users/qramo/Downloads/all_events_final_enriched.RData"
...
if (file.exists(data_path)) {
  ...
  } else if (exists("data_for_models")) {
    data_for_models <- data_for_models
  } else {
    loaded_objects <- ls()
    data_objects <- loaded_objects[sapply(loaded_objects, function(x) {
      obj <- get(x)
      (is.data.table(obj) || is.data.frame(obj)) && nrow(obj) > 1000
    })]
    
    if (length(data_objects) > 0) {
      data_for_models <- get(data_objects[1])
    } else {
      stop("No suitable data object found in loaded file")
    }
  }
} else {
  if (exists("all_events_final_enriched", envir = .GlobalEnv)) {
    data_for_models <- get("all_events_final_enriched", envir = .GlobalEnv)
  } else if (exists("data_for_models", envir = .GlobalEnv)) {
    data_for_models <- get("data_for_models", envir = .GlobalEnv)
  } else {
    env_objects <- ls(envir = .GlobalEnv)
    suitable_objects <- env_objects[sapply(env_objects, function(x) {
      obj <- get(x, envir = .GlobalEnv)
      (is.data.table(obj) || is.data.frame(obj)) && 
      "diffusion" %in% names(obj) && 
      "structural_distance" %in% names(obj) &&
      nrow(obj) > 1000
    })]
    
    if (length(suitable_objects) > 0) {
      data_for_models <- get(suitable_objects[1], envir = .GlobalEnv)
    } else {
      stop("No suitable data found. Please ensure data is loaded.")
    }
  }
}
```
